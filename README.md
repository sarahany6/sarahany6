## Hi there 👋
<div align="center"> <!-- Typing animation --> <a href="https://github.com/YOUR-USERNAME"> <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=24&pause=1000&color=2EA3F2&center=true&vCenter=true&width=600&lines=Hi+there%2C+I'm+Sara+%F0%9F%91%8B;CS+Student+%7C+Data+%26+ML+Enthusiast;Building+frontend+projects" alt="Typing SVG" /> </a> </div>

### About Me
<div>
 <ul>
  <li>🎓 Third-year Computer Science student
  </li>
  <li>📊 Focused on Data Analytics,Data science, Machine learning, and Frontend Track
  </li>
 </ul>
</div>

### 💼 What I Do
<div>
 <ul>
  <li>🔄 Transform raw data into useful insights
</li>
  <li>📊 Design interactive dashboards
</li>
  <li>📈 Analyze business performance
</li>
  <li>🤖 Use AI models in predictions
</li>
  <li>📝 Deliver supportive reports that help drive decision-making
</li>
  <li>🎨 Build interactive, responsive frontend interfaces — from UI components to full web app features (Tailwind CSS, JavaScript/DOM)
</li>
 </ul>
</div>

### 🧰 My Services

## Data & Analytics
<div>
 <ul>
  <li>📊 Exploratory Data Analysis (EDA) — turning messy datasets into clear findings</li>
  <li>📈 Interactive dashboard design (Power BI) for sales, customer, and performance reporting</li>
  <li>🤖 Predictive modeling using machine learning (regression, classification, ensemble models like Random Forest & XGBoost)</li>
  <li>📝 Data storytelling & report writing to support business decisions</li>
  <li>🧹 Data cleaning, preprocessing, and feature engineering</li>
 </ul>
</div>

## Frontend Development
<div>
 <ul>
  <li>🎨 Building responsive, modern UIs with HTML, Tailwind CSS
  </li>
  <li>⚡ Interactive web features using JavaScript / DOM
  </li>
  <li>🧩 Turning designs into functional, user-friendly interfaces
  </li>
 </ul>
</div>

### 🔭 Current Projects
<div>
 <ul>
  <li>Students' Scores Analysis(https://github.com/sarahany6/Student_scores_performance)</li>
  <li>My first Portfolio(https://github.com/sarahany6/final-project)</li>
  <li>Sales & Customers Dashboards </li>
 </ul>
</div>

### 🛠️ Tech Stack
 <div>
 <ul>
  <li>Python</li>
  <li>SQL</li>
  <li>Numpy, Pandas</li>
  <li>Excel, Power BI, Tableau</li>
  <li>HTML,CSS,JS</li>
 </ul>
</div>

### 🌱 Currently Learning
<div>
 <ul>
  <li>Advanced SQL </li>
  <li>Advanced AI Models</li>
  <li>Enhance my skills in frontend track </li>
 </ul>
</div>

### 📫 Connect With Me
 
<div>
 <ul>
  <li>LinkedIn:(https://linkedin.com/in/sara-hany-9ba849320)</li>
  <li>Email:(mailto:sara6hany@gmail.com)</li>
 </ul>
</div> 





